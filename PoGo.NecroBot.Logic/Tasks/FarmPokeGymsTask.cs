...
namespace PoGo.NecroBot.Logic.Tasks
{
    public static class FarmPokeGymsTask
    {
        ...
        public static async Task Execute(ISession session, CancellationToken cancellationToken)
        {
            ...
            while (pokeGymsList.Any())
            {
                cancellationToken.ThrowIfCancellationRequested();

                //resort
                pokeGymsList =
                    pokeGymsList.OrderBy(
                        i =>
                            LocationUtils.CalculateDistanceInMeters(session.Client.CurrentLatitude,
                                session.Client.CurrentLongitude, i.Latitude, i.Longitude)).ToList();
                var pokeGym = pokeGymsList[0];
                pokeGymsList.RemoveAt(0);

                var distance = LocationUtils.CalculateDistanceInMeters(session.Client.CurrentLatitude,
                    session.Client.CurrentLongitude, pokeGym.Latitude, pokeGym.Longitude);
                var fortInfo = await session.Client.Fort.GetGymDetails(pokeGym.Id, pokeGym.Latitude, pokeGym.Longitude);
                ...
                do
                {
                    ...
                    fortSearch =
                        await session.Client.Fort.SearchFort(pokeGym.Id, pokeGym.Latitude, pokeGym.Longitude);
                    ...
                    else
                    {
                        session.EventDispatcher.Send(new FortUsedEvent
                        {
                            ...
                            Latitude = pokeGym.Latitude,
                            Longitude = pokeGym.Longitude,
